...
Function Get-Problems([parameter(Mandatory = $true)]$Type) {
  ...
  elseif ($Type -eq 'MissingType') {
    $ticketSearch = Get-CWMTicket -condition "status/Name = `">Closed`" and closedDate > [$ticketDate] and type/name = null and ParentTicketId = null" -pageSize 1000
    $messageTitle = "$date - Ticket Type Missing"
    $uniqueMembers = $ticketSearch.closedBy | Sort-Object | Get-Unique
  }
  elseif ($Type -eq 'CatchAll') {
    $ticketSearch = Get-CWMTicket -condition "status/Name = `">Closed`" and closedDate > [$ticketDate] and company/name = `"Catchall`" and ParentTicketId = null" -pageSize 1000
